...
const cohere = new CohereClient({
    token:"lgJryDFi80g1PKvcOJ6Yf06T2FFduqFoHMfLviKT",
});
...
exports.addResume = async(req,res) =>{
    ...
    const match = result.match(/Score:\s*([\s\S]*)/);
...
res.status(200).json({ message: "Your analysis are ready", data: newResume});
    ...
    exports.getAllResumesForUser = async (req,res) => {
        try{
         ...
         let resumes = await ResumeModel.find({user:user}).sort({createdAt: -1}).populate('user');
